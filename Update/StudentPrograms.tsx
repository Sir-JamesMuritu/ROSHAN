...
const StudentPrograms = () => {
  ...
  const canEnroll = profileCompletion >= 80;
            ...
            <CardContent className="flex items-start gap-3 p-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-amber-700 dark:text-amber-300 mt-1">
                  Your profile is {profileCompletion}% complete. You need at least 80% to enroll in programs.
                </p>
                <div className="mt-2">
                  <Progress value={profileCompletion} className="h-2" />
                </div>
                <Link to="/profile" className="inline-block mt-2 text-xs font-medium text-primary underline">
                  Complete Profile →
                </Link>
              </div>
            </CardContent>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {programs.map((program: any, i: number) => {
            ...
            if (!canEnroll && !isEnrolled) {
              buttonLabel = "Complete Profile First";
              buttonDisabled = true;
            } else if (isEnrolled && !isRejected) {
